...
use lib "/exec/apps/bin/lib/perl5/JN";
my $defaultDelta = 0.6;
		...
		if ($j<$halfy) {
			...
			for($i=$right;$i<$rightEdge{$j};$i++) {
				if(isTestDie($MatrixData[$i][$j])) {
					...
					add2Edge(\@part2,$i,$nj,$MatrixData[$i][$nj]);
				}
			}
			...
			for($i=$left;$i>$leftEdge{$j};$i--) {
				if(isTestDie($MatrixData[$i][$j])) {
					...
					# should be test die from here
					add2Edge(\@part1,$i,$nj,$MatrixData[$i][$nj]);
				}
			}
			...
			for($i=$rightEdge{$j}-1;$i>$right;$i--) {
				if(isTestDie($MatrixData[$i][$j])) {
					...
					add2Edge(\@part3,$i,$nj,$MatrixData[$i][$nj]);
				}
			}
			...
			for($i=$leftEdge{$j}+1;$i<$left;$i++) {
				if(isTestDie($MatrixData[$i][$j])) {
					...
					# is test die from now
					add2Edge(\@part4,$i,$nj,$MatrixData[$i][$nj]);
				}
			}
		}
	}
